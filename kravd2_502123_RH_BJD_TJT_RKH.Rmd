...
pdata_s1 <- tibble(x = x_values,
                    size = 1,
                    y = dbinom(x_values, size, prob))
pdata_s2 <- tibble(x = x_values,
                    size = 2,
                    y = dbinom(x_values, size, prob))
pdata_s3 <- tibble(x = x_values,
                    size = 3,
                    y = dbinom(x_values, size, prob))
pdata_s10 <- tibble(x = x_values,
                    size = 10,
                    y = dbinom(x_values, size, prob))

pdata_s <- rbind(pdata_s1
                 # pdata_n20,
                 # pdata_n50,
                 # pdata_n100
                 )
...
ggplot() +
  geom_bar(data=pdata_s, aes(x, y), stat = "identity") +
  # ylab("pmf") +
  facet_wrap(~ size, labeller = label_both) #+

# df <- data.frame(x, dbinom=dbinom(x, size, prob)) 
# 
# ggplot(df, aes(x, dbinom)) +
